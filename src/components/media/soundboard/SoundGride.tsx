// src/components/media/soundboard/SoundGrid.tsx
...
const SoundGrid: React.FC<SoundGridProps> = ({
  ...
  const [assigningHotkeyId, setAssigningHotkeyId] = useState<string | null>(null); // NEW
  ...
  useEffect(() => {
    ...
    try {
      ...
      audioEngine.current.setMasterVolume(masterVolume);
    } catch (e) {
  ...
  const handleSoundPlay = useCallback((sound: SoundItem) => {
    ...
    // Exklusives Playback: andere stoppen
    if (!sound.playing && exclusivePlayback) {
      for (const s2 of sounds) {
        if (s2.id !== sound.id && s2.playing) {
          engine.stopSound(s2.id, s2.fadeOut);
          onSoundStop(s2.id);
        }
      }
    }
  ...
  }, [onSoundPlay, onSoundStop, quantumMode, mouseX, mouseY, sounds, exclusivePlayback]);
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      e.preventDefault();
      const disallowed = ['shift', 'control', 'alt', 'meta', 'capslock', 'tab', 'escape'];
      const key = e.key.toLowerCase();
      if (disallowed.includes(key)) return;
      onHotkeyAssign?.(assigningHotkeyId, key);
      setAssigningHotkeyId(null);
    };
    window.addEventListener('keydown', handler);
    return () => window.removeEventListener('keydown', handler);
  }, [assigningHotkeyId, onHotkeyAssign]);
        ...
        <AnimatePresence>
          {hoveredSound === sound.id && !sound.locked && (
            ...
            >
              {/* NEW: Hotkey-Recorder */}
              <IconBtn
                onClick={() => setAssigningHotkeyId(sound.id)}
                active={assigningHotkeyId === sound.id}
                activeBg="rgba(0,255,204,0.2)"
                activeColor="#00ffcc"
