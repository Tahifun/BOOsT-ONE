...
export default async function authLogin(req: Request, res: Response) {
  try {
    ...
    if (!user || !user.passwordHash) return res.status(401).json({ ok: false, message: "UngÃ¼ltige Zugangsdaten." });

    const ok = await bcrypt.compare(password, user.passwordHash);
    if (!ok) return res.status(401).json({ ok: false, message: "UngÃ¼ltige Zugangsdaten." });
