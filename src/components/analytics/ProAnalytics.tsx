  ...
  return (
    <ProFeatureWrapper featureName="pro_analytics">
      <div className="overlay-widget-card" role="region" aria-label="Pro Analytics">
        <div className="widget-icon">ï¿½Y"^</div>
        <div className="widget-title">Pro Analytics</div>
        <div className="widget-desc" style={{ marginBottom: 10 }}>
          Tiefere Einblicke in Zuschauer, Follows, Coins & Chat ï¿½?" mit Vergleich Ã¼ber Zeit.
        </div>
          ...
          {(Object.keys(RANGES) as RangeKey[]).map((k) => (
            <button
              ...
              style={{
                ...
                background:
                  range === k
                    ? 'linear-gradient(90deg,#21ffd9,#1979f8)'
                    : 'linear-gradient(90deg,#22e5ff,#18bbd0)',
                color: '#012',
                fontWeight: 800,
        ...
        >
          <MetricCard
            label="Peak Viewers"
            value={stats ? fmt(stats.peakViewers) : '—'}
            loading={loading}
          />
          <MetricCard
            label="ï¿½~ Viewers"
            value={stats ? fmt(stats.avgViewers) : '—'}
            loading={loading}
          />
          <MetricCard
            label="Neue Follower"
            value={stats ? fmt(stats.newFollowers) : '—'}
            loading={loading}
          />
          <MetricCard label="Coins" value={stats ? fmt(stats.coins) : '—'} loading={loading} />
          <MetricCard
            label="Chat-Nachrichten"
            value={stats ? fmt(stats.chatMessages) : '—'}
            loading={loading}
          />
          <MetricCard
            label="Engagement"
        ...
        >
          <SeriesCard
            title="Zuschauer-Verlauf"
            data={stats?.viewersSeries || []}
            loading={loading}
          />
          <SeriesCard
            title="Follower-Zuwachs"
            data={stats?.followersSeries || []}
            loading={loading}
          />
...
const MetricCard: React.FC<{ label: string; value: string; loading?: boolean }> = ({
  ...
  >
    ...
    <div
      style={{ fontSize: 22, fontWeight: 900, color: '#18ffe6', textShadow: '0 0 8px #18ffe666' }}
    >
      {loading ? 'ï¿½?ï¿½' : value}
    </div>
  </div>
...
const SeriesCard: React.FC<{ title: string; data: Point[]; loading?: boolean }> = ({
  title,
  data,
  loading,
}) => (
  <div className="overlay-widget-card" style={{ padding: 14 }}>
    ...
    {loading ? (
      <div style={{ opacity: 0.8 }}>Lade ï¿½?ï¿½</div>
    ) : data?.length ? (
