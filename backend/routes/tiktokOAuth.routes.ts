...
const TIKTOK_SCOPE =
  process.env.TIKTOK_OAUTH_SCOPE ?? 'user.info.basic';
const STATE_SECRET =
  process.env.TIKTOK_OAUTH_STATE_SECRET ?? 'dev_state_secret';
...
router.get('/callback', async (req: Request, res: Response) => {
  try {
    ...
    // Aufs Frontend weiterleiten (hier kannst du deine Zielseite setzen)
    return res.redirect('http://localhost:5173/settings?connected=tiktok');
  } catch (e: any) {
