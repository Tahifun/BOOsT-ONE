...
// -> Vereinheitlicht Tiers (unterstÃ¼tzt 'free/pro/enterprise' und 'FREE/PRO/ENTERPRISE')
...
export function requireTier(required: AnyTier | AnyTier[]): RequestHandler {
  ...
  return function (req: AuthenticatedRequest, res: Response, next: NextFunction) {
    ...
    return res.status(403).json({ message: "Dieses Feature ist fÃ¼r dein Tier nicht freigeschaltet" });
  };
}
