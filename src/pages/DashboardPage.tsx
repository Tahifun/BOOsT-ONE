...
const DashboardPage: React.FC = () => {
  ...
  const Orb: React.FC<{ value: string; label: string; color: string }> = ({
    value,
    label,
    color,
  }) => (
    <div className="quantum-orb">
      <div className="orb-container">
        ...
        <div className="orb-rings">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="orb-ring" style={{ ['--ring-delay' as any]: `${i * 0.3}s` }} />
          ))}
        </div>
      </div>
    ...
    <div className={`epic-dashboard ${theme === 'epic' ? 'theme-epic' : ''}`}>
      ...
      <div className="dashboard-header">
        <div className="header-content">
          <div className="welcome-section">
            <h1 className="glitch-text" data-text="COMMAND CENTER">
              COMMAND CENTER
            </h1>
          ...
          <Link to="/overlay/editor" className="overlay-editor-button">
            ...
            <span className="button-text">Ã¯Â¿Â½YZÃ¯Â¿Â½ OVERLAY EDITOR</span>
      ...
      <div className="dashboard-grid">
        <div className="grid-section additional-stats">
          <HolographicCard title="SPOTIFY" icon={'\u{1F3B5}'} delay={0} className="spotify-card">
            <div id="spotify-card">
          ...
          <HolographicCard title="BOOST MOMENTS" icon="Ã¯Â¿Â½sÃ¯Â¿Â½" delay={100}>
            <BoostMoments />
          </HolographicCard>
        </div>

        <div className="grid-section primary-stats">
          <HolographicCard title="VIEWER ANALYTICS" icon="Ã¯Â¿Â½Y'Ã¯Â¿Â½" delay={200}>
            <ViewerStats />
          </HolographicCard>

          <HolographicCard title="FOLLOWER MATRIX" icon="Ã¯Â¿Â½Y'Ã¯Â¿Â½" delay={300}>
            <FollowerStats />
          </HolographicCard>

          <HolographicCard title="REVENUE STREAM" icon="Ã¯Â¿Â½Y'Z" delay={400}>
            <CoinStats />
          </HolographicCard>
        </div>

        <div className="grid-section social-stats">
          <HolographicCard title="TIKTOK NEURAL NET" icon={'\u{1F4F1}'} delay={500}>
            <TikTokStats />
          </HolographicCard>
          <HolographicCard title="SENTIMENT ANALYSIS" icon={'\u{1F4AC}'} delay={600}>
            <SentimentGraph />
          </HolographicCard>
          <HolographicCard title="PRO ANALYTICS" icon={'\u{1F4CA}'} delay={700} pro>
            <ProAnalytics />
          </HolographicCard>
        </div>

        <div className="grid-section advanced-stats">
          <HolographicCard title="VIEWER TIMELINE" icon={'\u{1F553}'} delay={800}>
            <ViewerGraph />
          </HolographicCard>
          <HolographicCard title="GROWTH TRAJECTORY" icon={'\u{1F680}'} delay={900}>
            <FollowerTrend />
          </HolographicCard>
          <HolographicCard title="SESSION MATRIX" icon={'\u23F0'} delay={1000}>
            <SessionTimeline />
          </HolographicCard>
        </div>

        <div className="grid-section engagement-stats">
          <HolographicCard title="HEAT SIGNATURE" icon={'\u{1F525}'} delay={1100}>
            <HeatmapStats />
          </HolographicCard>
          <HolographicCard title="REPLAY VALUE" icon={'\u{1F501}'} delay={1200}>
            <RewatchStats />
          </HolographicCard>
          <HolographicCard title="CHAT VELOCITY" icon={'\u{1F4AC}\u26A1'} delay={1300}>
            <ChatPeakGraph />
          </HolographicCard>
        </div>
      </div>

      {/* Data Stream (unverÃƒÂ¤ndert) */}
              ...
              style={
                {
                  ...
                  ['--color' as any]:
                    b.value > 70 ? '#00ffff' : b.value > 40 ? '#a855f7' : '#ec4899',
