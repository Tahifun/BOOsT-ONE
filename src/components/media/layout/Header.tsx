...
export const Header: React.FC<HeaderProps> = ({ onSearch, className = '' }) => {
  ...
  // Debounced Search + Demo-VorschlÃ¤ge
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchQuery.trim().length > 2) {
        setSearchSuggestions([
          `${searchQuery} â€“ Videos`,
          `${searchQuery} â€“ Clips`,
          `${searchQuery} â€“ Screenshots`,
          `${searchQuery} â€“ Sounds`,
        ]);
  ...
  const handleSearch = (query: string) => {
    ...
    if (onSearch) {
      ...
      // Fallback: navigiert zur Media-Seite und Ã¼bergibt die Suche als Query
        ...
        <div className={`header-search ${isSearchFocused ? 'focused' : ''}`}>
          <div className="search-container">
            ...
            <span id="search-hint" className="sr-only">
              DrÃ¼cke Enter zum Suchen oder nutze die VorschlÃ¤ge.
            </span>
              ...
              <div className="search-suggestions" role="listbox" aria-live="polite">
                {searchSuggestions.map((suggestion, index) => (
                  <button
                    ...
                    type="button"
                  >
                    <span className="suggestion-icon">ðŸ”Ž</span>
          ...
          <div className="user-badge">
            <div className="user-avatar">
              {user?.avatar ? (
                ...
                <span className="avatar-placeholder" aria-hidden="true">ðŸ‘¤</span>
              )}
            </div>
            <div className="user-info">
              ...
              {tier === 'PRO' ? (
                <span className="pro-badge" aria-label="PRO-Status">
                  <span className="pro-star" aria-hidden="true">âœ¨</span>
